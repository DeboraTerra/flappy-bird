...
class Jogador: # cria uma classe jogador
    def __init__(self, tela, x, y): # def indica uma função, e __init__ é uma palavra reservada que sempre é chamada quando usamos a classe. self tambem é reservado e indica que cada objeto criado tera informações próprias, e x e y são variáveis que receberão a posição onde é criado o jogador e tela é a tela em que desenhamos nosso jogador
        ...
        self.contador = 0 #cria uma variavel para contar quantas atualizações necessarias antes de mudar de imagem
        ...
        self.listaImagens = [] # cria uma lista vazia que receberá as imagens do jogador
        ...
        for i in range(3): # laço que vai pegar as 3 imagens do jogador e adicionar na lista
            ...
            # a função load serve para pegar as imagens e guardar na variavel imagem
    ...
    def desenhar(self): # cria uma função que desenha o jogador
        ...
        if self.contador > 5: # verifica se o contador é maior que 5
            ...
            self.imagemAtual = (self.imagemAtual + 1) % 3 # adiciona 1 na variavel imagemAtual e pega o resto da divisão por 3 usando o sinal %, que pode apenas, 0, 1 ou 2.
        self.tela.blit(self.listaImagens[self.imagemAtual], self.posicao) # usa a função blit para desenhar a imagem na tela

    def atualizar(self):
        self.velocidadeAtual = min(self.velocidadeAtual + self.gravidade, self.velocidadeMaxima) # adiciona a velocidade atual
        self.posicao = [self.posicao[0] , self.posicao[1]+ self.velocidadeAtual] # adiciona a velocidade atual ao Y
        self.rect = pygame.Rect(self.posicao, self.tamanho) # atualiza a rect para o novo tamanho do jogador
